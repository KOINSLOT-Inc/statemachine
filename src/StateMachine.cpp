...
void StateMachine::transition(State targetState) {
  ...
  uint8_t i = 0;
  while (currentAncestry[i] && targetAncestry[i] &&
         currentAncestry[i] == targetAncestry[i]) {
    i++;
  }
...
Result StateMachine::handle(Event event) { return processEvent(state, event); }

Result StateMachine::processEvent(State targetState, Event event) {
  Result result = (this->*targetState)(event);

  switch (result.type) {
  case RESULT_UNHANDLED: { // pass to parent to handle
    State parent =
        (this->*targetState)(Event{.signal = EVENT_REQUEST_PARENT}).target;
    if (!state)
